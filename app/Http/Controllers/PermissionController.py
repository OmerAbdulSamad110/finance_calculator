...
class PermissionController:
    ...
    async def index(
        ...
        data.list = [
            PermissionDetailResponse(**self.__formatItem(permission))
    ...
    async def show(
        ...
        return JsonResponse(
            data={
                "permission": PermissionDetailResponse(**self.__formatItem(permission))
            }
        )
    ...
    async def update(
        ...
        permission = await self.__findItemForWrite(db, id)
    ...
    def __formatItem(self, permission: RowMapping) -> dict:
        item = {
            "id": permission["Permission"].id,
            "label": permission["Permission"].label,
    ...
    async def __findItemForWrite(self, db: AsyncSession, id: int):
        stmt = await db.execute(select(Permission).where(Permission.id == id))
