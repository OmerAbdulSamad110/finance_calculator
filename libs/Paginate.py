...
class Paginate:
    ...
    async def offset(
        ...
        # Get the total number of items
        total_items = await Paginate.__getItemsCount(db, query)
        ...
        result = await db.execute(
            query.offset(offset).limit(paginate_request.list_size)
        )
    ...
    async def cursor(
        ...
        paginate_request: CursorPaginateRequest,
        ...
        if paginate_request.cursor is not None:
            try:
                cursor_value = Encryption.decode(
        ...
        if last_item is not None:
            ...
            if next_cursor_value is None:
                raise ValueError(f"Invalid order_by: {order_by}")
            has_more = True
            next_cursor = Encryption.encode(
                next_cursor_value, f"{order_by}|{order_dir}|{list_size}"
            )
...
PaginationDependency = Annotated[PaginateRequest, Depends()]
CursorPaginationDependency = Annotated[CursorPaginateRequest, Depends()]
